...
int main(int argc, char **argv){
   ...
   const int lim_year = atoi(argv[2]);
   int lim_show = 0;
   
   if(argv[3] != NULL){
     lim_show = atoi(argv[3]);
   }
   ...
   while(ifs && getline(ifs, temp)){
     ...
     int year = atoi(ret_string[2].c_str());
     if(year >= lim_year){
       document_size++;
   ...
   ostringstream oss;
   oss << argv[1] <<".mutual_info_" <<argv[2] << "_" << argv[3];
   ofstream ofs;
   ...
   for(x = n.begin(); x != n.end(); ++x){
     ...
     for(multimap<double, int>::reverse_iterator i = mutual_info.rbegin(); i != mutual_info.rend(); ++i){
       tmp << " " << i->second << ":" << n[j_id][i->second];
       counter++;
       if(counter >= lim_show && lim_show != 0){break;}
     }
     ofs << tmp.str() << endl;
   }
   ofs.close();
